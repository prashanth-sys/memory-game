...
class MemoryMatrix extends Component {
  ...
  onClickCell = clickedId => {
    ...
    const clickedIndex = highlightedIndices.findIndex(
      cell => cell.id === clickedId,
    )

    if (clickedIndex !== -1 && !clickedIndices.includes(clickedIndex)) {
      console.log('matched')

      this.setState(prevState => ({
        clickedIndices: [...prevState.clickedIndices, clickedIndex],
      }))
